#!/usr/bin/perl
#
# insert_end_text_en_messages.pl
# End text rebuilder for "Capcom vs. SNK 2" on the SEGA Dreamcast.
#
...
# Set input file.
my $input_file = "MESE_DM_NEW.BIN.xlsx";
my $message_file = "../gdi_original_extracted/MESE_DM.BIN";

# Set output files.
my $output_file = "MESE_DM.BIN";

# Store pre-text/pointer section of message data.
my $pre_text_data = &read_bytes_at_offset($message_file, 256, 4);

# Store pointer table from message data.
my $pointer_table = &read_bytes_at_offset($message_file, 3016, 14028);
my @pointer_table_entries = $pointer_table =~ /(.{1,8})/gs;

# Store offset of text's starting position in message data (0x104).
my $text_position = 260;

# Store length (in bytes) of original text portion (0x35C8).
my $original_text_size = 13768;

# Store value of the the pointer in original header for purposes of recalcuation.
my $header_pointer = 16044;

# Set maximum output file size to 19844 bytes.
my $maximum_file_size = 19844;

# Declare empty hash for storing string data.
my %strings = ();

# Status message.
print "\n\"Capcom vs. SNK 2\" end text rebuilder for the SEGA Dreamcast.\n";
...
for(my $i = 1; $i < scalar(@spreadsheet_rows); $i ++)
{
	...
	my $translation = decode_entities($spreadsheet_rows[$i][4]);
...
# Recalculate the pointer that begins the file's header.
my $new_header_pointer = $header_pointer + $size_difference;
$new_header_pointer = &endian_swap(&decimal_to_hex($new_header_pointer, 4));

# Prepend both pointers to header.
$pre_text_data = $new_header_pointer . $pre_text_data;
...
for(my $i = 0; $i < scalar(@pointer_table_entries); $i ++)
{
	...
	else
	{
		...
		if($current_pointer_decimal >= 260)
		{
