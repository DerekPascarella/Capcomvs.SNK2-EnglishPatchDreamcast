...
# Set input file.
my $input_file = "../gdi_original_extracted/MESE_DM.BIN";

# Set output files.
my $output_file = "MESE_DM.BIN.xls";
...
for(my $i = 0; $i < scalar(@text_bytes); $i ++)
{
	...
	$strings{$string_count}{'Original Text'} = decode('shiftjis', pack('H*', $byte_string));
}
...
sub write_spreadsheet
{
	...
	$worksheet->write(0, 3, "Original Text", $header_format);
	$worksheet->write(0, 4, "Revised Text", $header_format);
	...
	{
		my $japanese_text = $spreadsheet_data{$string_number}{'Original Text'};
